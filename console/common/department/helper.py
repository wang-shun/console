...
from console.common.account.models import (
    Account,
    AcountStatus
)
...
# from console.console.nets.helper import delete_user_base_net
from .models import (
    Department,
    Organization
)
...
class DepartmentService(object):
    ...
    def create(cls, name, description, parent_department_id):
        ...
        try:
            parent_department = Department.objects.get(department_id=parent_department_id)
    ...
    def get_tree(cls, department_id=''):
        ...
        member_stat = Organization.objects.values('department_id').annotate(total=Count('department_id'))
        member_map = {x['department_id']: x['total'] for x in member_stat}
        ...
        def find_sub_department(parent_department):
            ...
            for sub_department in direct_sub_departments:
                result.append({
                    ...
                    'total_member': member_map.get(sub_department.id, 0),
                })
        ...
        try:
            ...
            all_sub_departments = Department.objects.filter(path__startswith=department.path, deleted=False)
            # TODO: optimize here repeated code
            return [{
                'name': department.name,
                'id': department.department_id,
                'children': find_sub_department(department),
                'total_member': member_map.get(department.id, 0)
            }]
    ...
    def get_root(cls, department_id):
        ...
        try:
            ...
            return Department.objects.get(department_id=root_department_id)
    ...
    def get_members(cls, department_id):
        ...
        :return: User构成的list
        """
        members_id = Organization.objects.filter(department__department_id=department_id) \
            .values_list('member', flat=True)
        members = Account.objects.filter(user__in=members_id, deleted=False).exclude(name='system_image')
        return members
    ...
    def remove(cls, department_id):
        ...
        try:
            ...
            DepartmentMemberService.join(member_list, root_department_id)
            Department.objects.filter(department_id=department_id).update(deleted=True, delete_datetime=now())
            return True
        except Exception as e:
            logger.debug(e.message)
            return False
    ...
    def remove_all(cls):
        try:
            Account.objects.all().update(department=None)
            Department.objects.all().update(parent_department=None)
...
class DepartmentMemberService(object):
    ...
    def enable(cls, member_id_list):  # TODO:  parameter use members: User's list
        ...
        """
        try:
            Account.objects.filter(user__username__in=member_id_list).update(status=AcountStatus.ENABLE)
            accounts = Account.objects.filter(user__username__in=member_id_list).all()
            return accounts
        except Exception as e:
            logger.error(e.message)
            return None
    ...
    def disable(cls, member_id_list):  # TODO:  parameter use members: User's list
        ...
        """
        try:
            Account.objects.filter(user__username__in=member_id_list).update(status=AcountStatus.DISABLE)
            accounts = Account.objects.filter(user__username__in=member_id_list)
            return accounts
        except Exception as e:
            logger.error(e.message)
            return None
    ...
    def join(cls, members, department_id):
        ...
        """
        try:
            target_department = Department.objects.get(department_id=department_id)
            parent_department_ids = target_department.path.split(',')
            parent_departments = Department.objects.filter(department_id__in=parent_department_ids).all()
            members = members if isinstance(members, list) else [members]
            logger.info('%s was move into %s' % (members, target_department))
            for member in members:

                Organization.objects.filter(member=member).delete()
                # 更新member的所有上级部门
                ...
                # 更新member的直接上级部门
                Account.objects.filter(user=member).update(department=target_department)
            return True
        except Exception as e:
            logger.debug(e.message)
            return False
    ...
    def to_dict(cls, member, detail=False):
        ...
        if detail:
            ...
            if member.type in AccountType.PORTAL_TYPES:
                detail_info = dict(
                    main_name=member.name,
                    main_phone=member.phone,
                    backup_name=member.backup_name or '',
                    backup_phone=member.backup_phone or '',
                    company_name=member.company_name or '',
                    company_website=member.company_website or '',
                    company_addr=member.company_addr or ''
                )
