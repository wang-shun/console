...
(function($) {
	...
	$.slidebars = function(options) {
		...
		var settings = $.extend({
			siteClose: true, // true or false - Enable closing of Slidebars by clicking on #cloudin-site.
		...
		if (ua.match(/Android/)) { // Detect for Android in user agent string.
			...
			if (android < 3) $('html').addClass('cloudin-android'); // Add 'cloudin-android' helper class for unfixing elements.
		}
		...
		// Site Container
		if (!$('#cloudin-site').length) $('body').children().wrapAll('<div id="cloudin-site" />'); // Check if content is wrapped with cloudin-site, wrap if it isn't.
		var $site = $('#cloudin-site'); // Cache the selector.
		if (!$site.parent().is('body')) $site.appendTo('body'); // Check its location and move if necessary.
		$site.addClass('cloudin-slide'); // Add animation class.

		// Left Slidebar
		if ($('.cloudin-left').length) { // Check if the left Slidebar exists.
			var $left = $('.cloudin-left'), // Cache the selector.
			leftActive = false; // Used to check whether the left Slidebar is open or closed.
			if (!$left.parent().is('body')) $left.appendTo('body'); // Check its location and move if necessary.
			if (android && android < 3) $left.addClass('cloudin-static'); // Add static class for older versions of Android.
			if ($left.hasClass('cloudin-width-custom')) $left.css('width', $left.attr('data-cloudin-width')); // Set user custom width.
		}

		// Right Slidebar
		if ($('.cloudin-right').length) { // Check if the right Slidebar exists.
			var $right = $('.cloudin-right'), // Cache the selector.
			rightActive = false; // Used to check whether the right Slidebar is open or closed.
			if (!$right.parent().is('body')) $right.appendTo('body'); // Check its location and move if necessary.
			if (android && android < 3) $right.addClass('cloudin-static'); // Add static class for older versions of Android.
			if ($right.hasClass('cloudin-width-custom')) $right.css('width', $right.attr('data-cloudin-width')); // Set user custom width.
		}
		...
		function setMinHeights() {
			...
			if ($left && $left.hasClass('cloudin-static')) $left.css('minHeight', minHeight);  // Set minimum height to the left Slidebar.
			if ($right && $right.hasClass('cloudin-static')) $right.css('minHeight', minHeight);  // Set minimum height to the right Slidebar.
		}
		setMinHeights(); // Set them

		// Control Classes
		var $controls = $('.cloudin-toggle-left, .cloudin-toggle-right, .cloudin-open-left, .cloudin-open-right, .cloudin-close');
		...
		function initialise() {
			...
			if (!settings.disableOver || (typeof settings.disableOver === 'number' && settings.disableOver >= windowWidth)) { // False or larger than window size.
				...
				$('html').addClass('cloudin-init'); // Add helper class.
				...
				$('html').removeClass('cloudin-init'); // Remove helper class.
		...
		$slide = $('.cloudin-slide'); // Cache all elements to animate.
		...
		$('html').addClass('cloudin-anim-type-' + animation); // Add animation type class.
		...
		function open(side) {
			...
			function proceed() {
				if (this.init && side === 'left' && $left) { // Slidebars is initiated, left is in use and called to open.
					...
					$('html').addClass('cloudin-active cloudin-active-left'); // Add active classes.
					...
					$('html').addClass('cloudin-active cloudin-active-right'); // Add active classes.
		...
		function close(link) {
			if (leftActive || rightActive) { // If a Slidebar is open.
				...
				setTimeout(function() { // Wait for closing animation to finish.
					$('html').removeClass('cloudin-active cloudin-active-left cloudin-active-right'); // Remove active classes.
		...
		$('.cloudin-toggle-left').on('touchend click', function(event) {
			input(event);
			toggle('left'); // Toggle left Slidebar.
		});

		// Slidebar Toggle Right
		$('.cloudin-toggle-right').on('touchend click', function(event) {
			input(event);
			toggle('right'); // Toggle right Slidebar.
		});

		// Slidebar Left Open
		$('.cloudin-open-left').on('touchend click', function(event) {
			input(event);
			if (!leftActive) open('left'); // Slidebar is closed, open it.
		});

		// Slidebar Right Open
		$('.cloudin-open-right').on('touchend click', function(event) {
			input(event);
			// /-- Added by Arjun --/ $('.logo').attr('style', 'margin-left: 250px;')
			if (!rightActive) open('right'); // Slidebar is closed, open it.
		});

		// Slidebar Close
		$('.cloudin-close').on('touchend click', function(event) {
			input(event);
			// /-- Added by Arjun --/ if (leftActive || rightActive) $('.logo').removeAttr('style')
			if (leftActive || rightActive) close(); // A Slidebar is open, close it.
		});

		// Slidebar Close via Link
		$('.cloudin-slidebar a').not('.cloudin-disable-close').on('click', function(event) {
